 ...
 */
export async function* create(svg, sizes) {
...
class IconDir extends Uint8Array {
  ...
  constructor(entryCount) {
    super(
      IconDir.#firstEntryByteOffset + IconDir.#entryByteLength * entryCount,
    );

    const dv = new DataView(this.buffer, 0, IconDir.#firstEntryByteOffset);
  ...
  writeEntry(index, opts) {
    const byteOffset =
      IconDir.#firstEntryByteOffset + IconDir.#entryByteLength * index;
